...
from data.data_utils import remove_outlier
from data.dsp import audio_to_label, compress
from data.mel import extract_linear_features, extract_mel_features, extract_mel_features_tts
from data.metadata import replace_augment_name
from data.stft import TacotronSTFT
from processors.encodec_tokenizer import extract_encodec_token
from utils.io_utils import has_existed, save_feature, save_torch_audio, save_txt
...
def __extract_utt_acoustic_features(dataset_output, cfg, utt):
  ...
  """
  from data import audio, f0, duration
...
def extract_utt_acoustic_features_tts(dataset_output, cfg, utt):
  ...
  with torch.no_grad():
    ...
    if cfg.preprocess.extract_linear_spec:
      from data.mel import extract_linear_features
    ...
    if cfg.preprocess.extract_mel:
      from data.mel import extract_mel_features
...
def extract_utt_acoustic_features_vocoder(dataset_output, cfg, utt):
  ...
  with torch.no_grad():
    ...
    if cfg.preprocess.extract_amplitude_phase:
      from data.mel import amplitude_phase_spectrum
